 ...
 */
 ...
 use rs_matter::data_model::cluster_temperature_control;
 use rs_matter::data_model::cluster_oven_mode;
 use rs_matter::data_model::core::IMBuffer;
 use rs_matter::data_model::device_types::DEV_TYPE_OVEN;
 use rs_matter::data_model::device_types::DEV_TYPE_TEMPERATURE_CONTROLLED_CABINET;
 ...
 fn run() -> Result<(), Error> {
     ...
     let temperature_control_cluster = cluster_temperature_control::TemperatureControlCluster::new(Dataver::new_rand(matter.rand()));
 
     let subscriptions = SUBSCRIPTIONS.uninit().init_with(Subscriptions::init());
 
     // Assemble our Data Model handler by composing the predefined Root Endpoint handler with our custom clusters
     let dm_handler = HandlerCompat(dm_handler(&matter, &temperature_control_cluster));
     ...
     let mut device = pin!(async {
         loop {
             ...
             temperature_control_cluster.set_temperature_setpoint(!temperature_control_cluster.get_temperature_setpoint());
             subscriptions.notify_changed();
 
             info!("Oven tempurature set to {}", temperature_control_cluster.get_temperature_setpoint());
         }
     });
 ...
 const NODE: Node<'static> = Node {
     ...
     endpoints: &[
         ...
         Endpoint {
             id: 1,
             device_types: &[DEV_TYPE_OVEN],
             clusters: &[],
         },
         Endpoint {
             id: 2,
             device_types: &[DEV_TYPE_TEMPERATURE_CONTROLLED_CABINET],
             clusters: &[descriptor::CLUSTER, cluster_temperature_control::CLUSTER, cluster_oven_mode::CLUSTER],
         },
     ],
 };
 ...
 fn dm_handler<'a>(
     matter: &'a Matter<'a>,
     cluster: &'a cluster_temperature_control::TemperatureControlCluster,
     ...
     (
         ...
         root_endpoint::eth_handler(0, matter.rand())
             ...
             .chain(2, cluster_temperature_control::ID, cluster),
     )
 }
